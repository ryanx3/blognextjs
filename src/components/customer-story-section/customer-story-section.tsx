...
const customerStories = [
  {
    content:
      "Criar minha loja com site.set foi a melhor decisão para o meu negócio. A plataforma é super intuitiva, e consegui colocar meus produtos a venda em poucos minutos",
    author: {
      name: "Annete Bones",
    ...
    <section className="container py-8 md:py-10">
      <div className="flex flex-col items-center gap-12">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {customerStories.map((customerStories) => (
            <div key={customerStories.author.name} className="flex flex-col gap-6 rounded-lg bg-gray-500 p-6 md:p-12">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <strong className="text-gray-200 text-sm">
                    {customerStories.author.name}
                  </strong>
                  <span className="text-gray-300 text-xs">
                    {customerStories.author.role}
                  </span>
                </div>
              </div>
            </div>
